...
setup_puppet() {
    if [ -x /opt/puppetlabs/bin/puppet ]; then
        ...
        if [ $REDHAT_RELEASE ]; then
            RELEASE_URL="https://yum.puppetlabs.com/puppet7/puppet-release-${REDHAT_RELEASE}.noarch.rpm"
            ...
            if [ $UBUNTU_VERSION ]; then
                APT_URL="https://apt.puppetlabs.com/puppet7-release-${UBUNTU_VERSION}.deb"
            fi
            if [ $DEBIAN_VERSION ]; then
                APT_URL="https://apt.puppetlabs.com/puppet7-release-${DEBIAN_VERSION}.deb"
            fi
